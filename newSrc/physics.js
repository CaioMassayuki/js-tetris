...
const isTetrominoOccupiedCell = (tetromino, row, column) =>
  tetromino[row][column] !== 0
...
export const hasCollision = avatar => {
  ...
  for (let row = 0; row < tetromino.length; row++) {
    for (let column = 0; column < tetromino[row].length; column++) {
      if (
        isTetrominoOccupiedCell(tetromino, row, column) &&
        (!Playfield.arena[row + avatar.position.y] ||
          Playfield.arena[row + avatar.position.y][
            column + avatar.position.x
          ] !== 0)
      ) {
        return true
      }
